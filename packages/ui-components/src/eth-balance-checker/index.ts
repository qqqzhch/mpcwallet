...
function getContract(provider: Provider | Signer, address?: string) {
  return new Contract(address || DEFAULT_CONTRACT_ADDRESS, BalanceCheckerABI, provider)
}

export async function getAddressBalances(provider: Provider | Signer, address: string, tokens: string[], options: Options = {}) {
...
export async function getAddressesBalances(provider: Provider | Signer, addresses: string[], tokens: string[], options: Options = {}) {
  const contract = getContract(provider, options.contractAddress)
